...
# ---------------------------------------------------------
MODEL_ONNX_PATH = "Model/model_repVit.onnx"
...
def predict():
    ...
    try:
        ...
        if onnx_session is not None:
            ...
            logits = onnx_session.run(None, {input_name: tensor_img.numpy()})[0]
            probs_np = np.exp(logits) / np.sum(np.exp(logits), axis=1, keepdims=True)
            probs_np = probs_np[0]
